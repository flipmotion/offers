...
$(document).ready(function () {
	...
	var Settings = {
   	...
   	container: function($field, validator) {
		...
		Validation: function Validation(form) {
			...
			$(form).find(select).select2({ theme: "bootstrap" }).change(function (e) {
				console.log(e.target.name);
				$(form).formValidation('revalidateField', e.target.name);
			}).end();

			$(form).find('[data-input="file"]').fileinput({
				uploadUrl: 'url',
				...
				console.log(file);
			}).end().find('[data-input="file"]').fileinput({
				uploadUrl: 'url',
				allowedFileExtensions: ["jpg", "png", "gif"],
				minImageWidth: 50,
				minImageHeight: 50
	...
	function doModal(heading, formContent, event) {
		...
		var that = this;
		heading = $(this).text();
		formContent = $(this).parents('[data-item="product"]').find('.product-item-vertical--name p').html();
		...
		html += '<button class="btn btn-default" type="button" data-modal="Cancel">Cancel</button>';
		html += '<button class="btn btn-primary" type="button" data-modal="OK">OK</button>';
		html += '</div>'; // content
		html += '</div>'; // dialog
		html += '</div>'; // footer
		...
		$("#DelItem").find('[data-modal="OK"]').on('click', function (e) {
			$(that).parents('[data-item="product"]').remove();
			$("#DelItem").modal('hide');
			$('#DelItem').on('hidden.bs.modal', function (e) {
				console.log(e.target);
				$(this).remove();
	...
	var OrderTrash = {
		...
		getDataItems: function getDataItems(trash) {
			...
			trash.on('submit', function (e) {
				...
				//array of price names
				//that.data.push($(name[i]).text());
				//that.data.push($(price[i]).text());
